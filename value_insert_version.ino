...
void loop() {
  if (Serial.avaiable()) {
    delay(100);
    lcd.clear();
    double a[5]={0};    輸入的五個數值依序分別為:
    for(int i=0;i<5;i++){  1.距離 2.預定時間 3.停紅燈時間 4.阻力係數 5.面積
while(true){
char tmp;
if(Serial.available())tmp=Serial.read();
    ...
    lcd.print("extra gas:");
    lcd.setCursor(0, 1);
lcd.print(((1/2*1.1691*a[3]*a[4]*(a[0]/(a[1]-a[2]))*(a[0]/(a[1]-a[2]))*a[1])/3600)
-1/2*1.1691*a[3]*a[4]*((a[0]/(a[1]-a[2]))+5)*((a[0]/(a[1]-a[2])+5))*a[1])/3600)/7300);               
lcd.print(“l”);
}}
